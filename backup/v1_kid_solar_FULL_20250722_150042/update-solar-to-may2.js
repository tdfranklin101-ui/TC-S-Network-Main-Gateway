/**
 * The Current-See - Update SOLAR Amounts to May 2, 2025
 * 
 * This script updates the SOLAR amounts based on the proper day count
 * from join date to May 2, 2025 (3:20 PM PDT)
 */
...
const FINAL_DATE = new Date('2025-05-02T15:20:00-07:00'); // 3:20 PM PDT on May 2, 2025
const SOLAR_PER_DAY = 1;
const SOLAR_DOLLAR_VALUE = 136000;
const DISPLAY_DATE = FINAL_DATE.toISOString().split('T')[0]; // Just the date part for display
...
function updateSolarAmounts() {
  console.log(`Updating SOLAR amounts based on correct day count to ${DISPLAY_DATE}...`);
  ...
  try {
    ...
    for (const member of members) {
      ...
      const days = daysBetween(joinDate, DISPLAY_DATE);
      ...
      if (currentSolar !== correctSolar) {
        ...
        console.log(`  Current: ${currentSolar} SOLAR, Updated: ${correctSolar} SOLAR`);
        ...
        member.last_distribution_date = DISPLAY_DATE;
        member.lastDistributionDate = DISPLAY_DATE;
    ...
    console.log(`SOLAR amounts updated for ${updateCount} members.`);
