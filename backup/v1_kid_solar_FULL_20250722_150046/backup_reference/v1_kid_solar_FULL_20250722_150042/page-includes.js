 ...
 */
function processSimpleIncludes(html, options = {}) {
  try {
    // Default paths for includes
    const headerPath = options.headerPath || path.join(__dirname, 'public', 'includes', 'header.html');
    const footerPath = options.footerPath || path.join(__dirname, 'public', 'includes', 'footer.html');
    ...
    // Read includes with caching
    const headerContent = readWithCache(headerPath, includesCache);
    const footerContent = readWithCache(footerPath, includesCache);
    const seoMetaContent = readWithCache(seoMetaPath, includesCache);
    ...
    console.error(`Error in processSimpleIncludes: ${err.message}`);
    return html;
  }
}
