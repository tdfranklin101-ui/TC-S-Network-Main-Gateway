...
The production database is the Replit-managed PostgreSQL accessed via `DATABASE_URL`. This is the single source of truth — all agent activity, artifacts (1,163+), marketplace ledger (3,526+ entries), bulletin board conversations, resale history, and the Solar minting ledger live here. Both dev and deployed environments share this same database. `PRODUCTION_DATABASE_URL` points to a legacy Neon database with stale/older data and should NOT be used by the app. Never switch the app to use `PRODUCTION_DATABASE_URL`.
