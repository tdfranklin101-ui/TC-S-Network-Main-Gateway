...
The platform uses Replit-managed PostgreSQL via `DATABASE_URL`. Dev and production each have their own separate database — they are NOT shared. The deployed/published app's `DATABASE_URL` is bound to the production database (where all live transactions, agents, artifacts, ledger entries, and Solar minting accumulate). The dev environment's `DATABASE_URL` is a separate development DB with much lower activity (only fires when manually triggered). When checking live data or debugging production issues, always query the production database via the read-only production replica — never assume dev data reflects production. `PRODUCTION_DATABASE_URL` points to a legacy Neon database with stale/older data and should NOT be used by the app.
