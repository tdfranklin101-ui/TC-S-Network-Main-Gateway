...
Manual triggers: `POST /api/agents/daily-tasks/trigger` (all agents), `POST /api/agents/daily-tasks/run-single` (single agent by code), `POST /api/agents/kid-solar/prompt` (Kid Solar → KID SOL action request). Automated: Daily cron at 4:00 AM UTC.
