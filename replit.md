...
**1-Agent-Per-Human Rule** (Mint Security): Every external agent must be sponsored by a verified human member via `sponsor_member_id`. Each human member can sponsor exactly one external agent. Agents cannot sponsor other agents. This prevents mass registration attacks on the Solar mint. Validation enforced at registration: sponsor must exist, must be human (not an agent), and must not have already sponsored another agent.
