...
class DidStreamingCapture {
  ...
  interceptDidStreamingData() {
    ...
    window.fetch = async function(...args) {
      ...
      if (typeof url === 'string' && (url.includes('d-id') || url.includes('agents') || url.includes('talks'))) {
        ...
        self.processStreamingResponse(clonedResponse, url);
      }
      ...
      if (url.includes('d-id') || url.includes('agent')) {
        ...
        ws.addEventListener('message', (event) => {
          try {
            ...
            self.captureWebSocketData(data, url);
          } catch (e) {
            // Text message
            if (event.data && event.data.length > 3) {
              self.captureWebSocketData({ text: event.data }, url);
            }
          }
        });
      }
  ...
  extractAgentId(data) {
    return data.agent_id || data.agentId || 'v2_agt_vhYf_e_C';
  }
