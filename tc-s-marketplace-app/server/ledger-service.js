...
class LedgerService {
  ...
  async executeTransaction(transaction) {
    ...
    this.balances.set(userId, newBuyerBalance);

    // Add to seller (minus marketplace fee)
    const marketplaceFee = amount * 0.05; // 5% marketplace fee
    ...
    this.balances.set(sellerId, newSellerBalance);

    // Record completed transaction
    transaction.status = 'completed';
    transaction.marketplaceFee = marketplaceFee;
    transaction.sellerAmount = sellerAmount;
  ...
  async getTransactionHistory(userId, limit = 50) {
    const userTransactions = Array.from(this.transactions.values())
      .filter(tx => tx.userId === userId || tx.sellerId === userId)
  ...
  async getMarketplaceStats() {
    ...
    const totalFees = Array.from(this.transactions.values())
      .filter(tx => tx.type === 'purchase' && tx.status === 'completed')
      .reduce((sum, tx) => sum + (tx.marketplaceFee || 0), 0);
