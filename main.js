...
{"products": [{"title": "...", "description": "...", "estimatedPriceUSD": 29.99, "source": "Amazon", "url": "https://..."}]}`
  ...
  // ONE-TIME BACKFILL: Generate real AI files for existing agent marketplace items
  if (pathname === '/api/ecosystem-test/generate-initial-stock' && req.method === 'POST') {
    try {
      const sessionId = getCookie(req, 'tc_s_session');
      if (!sessionId) { res.writeHead(401, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: false, error: 'Authentication required' })); return; }
      const session = await getSession(sessionId);
      if (!session || !session.userId) { res.writeHead(401, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: false, error: 'Valid session required' })); return; }

      const body = await parseBody(req);
      const batchSize = Math.min(parseInt(body.batchSize) || 10, 25);
      const categoryFilter = body.category || null;

      const aiEngine = require('./ai-creation-engine');
      const resetBudget = body.resetBudget !== false;
      if (resetBudget) aiEngine.resetBudget();

      let query = `SELECT id, title, description, category, price_solar, metadata::text as metadata_text, created_by_user_id
        FROM market_items 
        WHERE metadata::text LIKE '%ecosystemTest%'
          AND (image_url IS NULL OR image_url = '')
          AND status = 'ACTIVE'
          AND (metadata::jsonb->>'hasRealFile' IS NULL OR metadata::jsonb->>'hasRealFile' = 'false')`;
      const params = [];
      if (categoryFilter) {
      ...
      console.log(`\n🏭 [INITIAL-STOCK] Starting one-time file generation for ${items.length} agent items...\n`);

      const results = [];
      let successCount = 0, failCount = 0, skippedCount = 0;

      for (const item of items) {
        const { id: itemId, title, description, category, price_solar, metadata_text, created_by_user_id } = item;
        let meta = {};
        try { meta = JSON.parse(metadata_text); } catch(e) {}
        const agentName = meta.agent || 'agent_system';

        console.log(`🔄 [INITIAL-STOCK] Processing: "${title}" [${category}] by ${agentName}`);

        let creationResult = { success: false };
        let creationSource = 'none';

        try {
          creationResult = await aiEngine.generateArtifactContent(category, title, description || '', agentName);
          if (creationResult.success) {
            creationSource = 'ai-self-creation';
            console.log(`  🤖 AI generated: ${creationResult.creationMethod} (${creationResult.fileSize} bytes)`);
          }
        } catch (aiErr) {
          console.warn(`  ⚠️ AI error: ${aiErr.message}`);
        }

        if (!creationResult.success) {
          try {
            const webDiscovery = require('./web-source-discovery');
            creationResult = await webDiscovery.findFreeContent(category, title, description || '', agentName);
            if (creationResult.success) {
              creationSource = 'web-discovery';
              console.log(`  🌐 Web sourced: ${creationResult.creationMethod} (${creationResult.fileSize} bytes)`);
            }
          } catch (webErr) {
            console.warn(`  ⚠️ Web error: ${webErr.message}`);
          }
        }

        if (!creationResult.success || !creationResult.fileBuffer) {
          console.log(`  ⏭️ Skipped: "${title}" — no file generated`);
          results.push({ itemId, title, category, status: 'skipped', reason: creationResult.error || 'No file generated' });
          skippedCount++;
          continue;
        }
        ...
        const fileBuffer = creationResult.fileBuffer;
        const actualMime = creationResult.fileType || 'application/octet-stream';
        const originalFilename = creationResult.filename || `backfill-${Date.now()}.bin`;
        ...
        try {
          fileProcessingResult = await fileManager.processUpload(
            fileBuffer,
            { originalname: originalFilename, mimetype: actualMime, size: fileBuffer.length },
            { title, description: description || '', category, creatorId: String(created_by_user_id) }
          );
        } catch (processingError) {
          console.error(`  ❌ Three-copy error: ${processingError.message}`);
          results.push({ itemId, title, category, status: 'failed', reason: `Processing error: ${processingError.message}` });
          failCount++;
          continue;
        }

        if (!fileProcessingResult || !fileProcessingResult.success) {
          results.push({ itemId, title, category, status: 'failed', reason: 'File processing returned no success' });
          failCount++;
          continue;
        }
        ...
        try {
          analysis = await analyzeContentForPricing(fileBuffer, actualMime, {
            title, description: description || '', category, fileSize: fileBuffer.length, filename: originalFilename
          });
        } catch (analysisErr) {
          const fileSizeMB = fileBuffer.length / (1024 * 1024);
          analysis = { estimatedKwh: fileSizeMB * 0.01, solarAmount: (fileSizeMB * 0.01) / 4913, reasoning: 'Fallback pricing' };
        }
        ...
        if (!actualMime.startsWith('video/')) {
          try {
            aiCurationResult = await aiCurator.generateSmartDescription(
              fileBuffer, actualMime, { title, description: description || '', category }
            );
          } catch (curErr) { /* continue */ }
        }

        const baseSlug = generateSlug(title, originalFilename);
        let finalSlug = baseSlug;
        let slugCounter = 1;
        while (true) {
          const slugCheck = await pool.query('SELECT id FROM artifacts WHERE slug = $1', [finalSlug]);
        ...
        const finalPrice = parseFloat(price_solar) || 0.01;
        const finalDesc = (aiCurationResult && aiCurationResult.success && aiCurationResult.description) || description || '';
        ...
        try {
          ...
          const artifactResult = await pool.query(insertQuery, [
            artifactId, finalSlug, title, finalDesc,
            (aiCurationResult && aiCurationResult.success && aiCurationResult.category) || category,
            actualMime, analysis.estimatedKwh, finalPrice, 0, 'download',
            fileProcessingResult.tradeFile.url, String(created_by_user_id),
            fileProcessingResult.previewFile.thumbnailUrl || null, true,
            fileProcessingResult.masterFile.url,
            fileProcessingResult.previewFile.previewUrl || null,
            fileProcessingResult.tradeFile.url,
            fileProcessingResult.masterFile.size,
          ...
          await pool.query(`
            ...
            WHERE id = $3
          `, [
            fileProcessingResult.previewFile.thumbnailUrl || fileProcessingResult.previewFile.previewUrl || null,
            JSON.stringify({
              ...meta,
              artifactId: dbArtifactId, artifactSlug,
              creationSource, creationMethod: creationResult.creationMethod,
              uploadType: 'initial_stock_backfill', hasRealFile: true,
              masterFileUrl: fileProcessingResult.masterFile.url,
              tradeFileUrl: fileProcessingResult.tradeFile.url,
              fileType: actualMime, fileSize: fileBuffer.length
            }),
            itemId
          ]);

          console.log(`  ✅ Complete: "${title}" → artifact ${artifactSlug} (Master: ${fileProcessingResult.masterFile.size}B, Trade: ${fileProcessingResult.tradeFile.size}B)`);

          results.push({
            itemId, title, category, status: 'success',
            artifactId: dbArtifactId, artifactSlug,
            creationMethod: creationResult.creationMethod,
            creationSource, fileType: actualMime, fileSize: fileBuffer.length,
            masterSize: fileProcessingResult.masterFile.size,
            tradeSize: fileProcessingResult.tradeFile.size
          });
          successCount++;
        } catch (dbErr) {
          console.error(`  ❌ DB error for "${title}": ${dbErr.message}`);
          results.push({ itemId, title, category, status: 'failed', reason: `DB error: ${dbErr.message}` });
          failCount++;
        }
      ...
      const totalRemaining = await pool.query(
        `SELECT COUNT(*) as cnt FROM market_items WHERE metadata::text LIKE '%ecosystemTest%' AND (image_url IS NULL OR image_url = '') AND status = 'ACTIVE' AND (metadata::jsonb->>'hasRealFile' IS NULL OR metadata::jsonb->>'hasRealFile' = 'false')`
      );

      console.log(`\n🏭 [INITIAL-STOCK] Batch complete: ${successCount} success, ${failCount} failed, ${skippedCount} skipped, ${totalRemaining.rows[0].cnt} remaining\n`);
      ...
      res.end(JSON.stringify({
        ...
        remaining: parseInt(totalRemaining.rows[0].cnt),
        budget: aiEngine.getBudgetStatus(),
        results
      }));
    } catch (error) {
      console.error('Initial stock generation error:', error);
      res.writeHead(500, { 'Content-Type': 'application/json' });
