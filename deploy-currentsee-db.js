/**
 * The Current-See Custom Database URL Deployment Script
 * 
 * This script specifically handles the CURRENTSEE_DB_URL environment variable
 * which appears to be set in your deployment environment.
 */
...
// Initialize database connection with CURRENTSEE_DB_URL
async function initializeDatabase() {
  // Use CURRENTSEE_DB_URL if available, otherwise fall back to DATABASE_URL
  const dbUrl = process.env.CURRENTSEE_DB_URL || process.env.DATABASE_URL;
  
  if (!dbUrl) {
    log('Neither CURRENTSEE_DB_URL nor DATABASE_URL is set.', true);
    return false;
  }
  ...
  try {
    ...
    if (err.message.includes('ENOTFOUND') || err.message.includes('getaddrinfo')) {
      ...
      try {
        log(`Hostname: ${new URL(dbUrl).hostname}`, true);
      } catch (e) {
        log('Invalid URL format in database URL', true);
      }
...
const server = app.listen(PORT, async () => {
  log('=== The Current-See Deployment Server (Custom DB URL) ===');
