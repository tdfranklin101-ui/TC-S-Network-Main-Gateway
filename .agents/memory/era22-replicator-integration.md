...
- ArmOS `/api/engineer` **504s after ~12s on complex prompts** — the adapter retries 3× with backoff; orchestration may make exactly one further attempt after a cooldown only for classified transient failures. Store the failure to prevent model tool-loop amplification; never substitute synthetic engineering.
- A retryable mission can be resumed only with its original control token, must preserve its node preference, and must consume the normal per-IP/concurrency budget. **Why:** recovery must not let a temporary upstream outage multiply engineering load.
