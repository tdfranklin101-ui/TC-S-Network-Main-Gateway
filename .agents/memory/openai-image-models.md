...
- Use `model: 'gpt-image-2'` (current since July 2026; any gpt-image-* id works).
...
**How to apply:** any server-side runtime image generation (e.g. Layer 3 `/api/market/produce` preview images) must use a gpt-image-* model (gpt-image-2 as of July 2026) and read `b64_json`. The agent's build-time `generateImage` skill is separate and unrelated to runtime server calls.
